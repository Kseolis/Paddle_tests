...
import static im.mak.paddle.blockchain_updates.transactions_checkers.ethereum_invoke_transaction_checkers.EthereumInvokeMetadataAssertions.checkEthereumPaymentMetadata;
...
public class SubscribeEthereumInvokeLeaseGrpcTest extends BaseGrpcTest {
    ...
    @Test
    @DisplayName("subscribe ethereum invoke with Lease and WAVES payment")
    void subscribeEthereumInvokeWithLease() throws NodeException, IOException {
    ...
    private void assertionsCheck(EthereumInvokeTransactionSender txSender, int txIndex) {
        assertAll(
                ...
                () -> checkArgumentsEthereumMetadata(txIndex, 0, BINARY_BASE58, callerAddress),
                () -> checkEthereumPaymentMetadata(txIndex, 0, WAVES_STRING_ID, amountValue),
                () -> checkEthereumLeaseMetadata(txIndex, 0, testData.getCallerPublicKeyHash(), amountValue),
                ...
                () -> checkStateUpdateBalance(txIndex, 1, dAppAddressString, WAVES_STRING_ID, dAppBalanceWavesBeforeTx, dAppBalanceWavesAfterTx),
                () -> checkStateUpdateBeforeLeasing(txIndex, 1, callerAddress, 0, 0),
                () -> checkStateUpdateBeforeLeasing(txIndex, 0, dAppAddressString, 0, 0),
                () -> checkStateUpdateAfterLeasing(txIndex, 1, callerAddress, amountValue, 0),
                () -> checkStateUpdateAfterLeasing(txIndex, 0, dAppAddressString, 0, amountValue),
